...
The executable baseline covers the contracts marked Current baseline below. The
remaining rows record later boundary gates that require their own focused migration. Target
...
| `Owner: architecture boundary map - Frontends preserve candidates for MathHost` | Current baseline | FrontendHost | `scieqlint.frontend` -> `scieqlint.parse` |
| `Owner: architecture boundary map - MathHost consumes facts without frontend imports` | Current baseline | MathHost | `scieqlint.parse` -> `scieqlint.frontend` |
...
| `scieqlint.parse` | math grammar, AST, parser, normalization, printing | MathHost | `parse.ast`, `diag.model`, source-span types, Lark adapter | `scan`, `frontend`, `check`, `report`, `config.load`, SymPy | Not CompatibilityShell | No temporary exception; future contract `Owner: architecture boundary map - Parsers must not import scanners checks reporters config loaders or SymPy` |
| `scieqlint.policy` | output-profile support, code-cell language-catalog policy, and diagnostic severity selection | PolicyHost preview | `facts`, `diag.catalog`, `diag.model` | `frontend`, `scan`, `parse`, `check`, `report`, `cli`, `app`, `api` | Not CompatibilityShell | Preview scope only; suppression and baseline migration remain owned by R1-021A/R1-021B/R1-022/R1-023/R1-024. |
