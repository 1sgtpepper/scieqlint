...
R1-002B owns the executable baseline for these contracts. The target names use
the `Owner: architecture boundary map` prefix so failure output identifies the
owning boundary.

| Contract name | Status | Owning boundary | Forbidden direction covered |
|---|---|---|---|
| `Owner: architecture boundary map - CLI must not import internal analysis layers directly` | Existing contract; R1-002B owner-prefix update | CLI/API compatibility | `scieqlint.cli` -> `scieqlint.scan`, `scieqlint.parse`, `scieqlint.check` |
| `Owner: architecture boundary map - Reporters render only diagnostics` | Existing contract; R1-002B expansion and owner-prefix update | SchemaHost projections | `scieqlint.report` -> `scieqlint.io`, `scieqlint.source`, `scieqlint.scan`, `scieqlint.parse`, `scieqlint.check`, `scieqlint.frontend`, `scieqlint.engine` |
| `Owner: architecture boundary map - Engines consume query facts only` | R1-002B planned | EngineHost | `scieqlint.engine` -> `scieqlint.scan`, `scieqlint.frontend`, `scieqlint.io`, `scieqlint.source`, `scieqlint.report`, `scieqlint.cli`, `scieqlint.app`, `scieqlint.api` |
| `Owner: architecture boundary map - Config must not import runtime analysis or presentation layers` | R1-002B planned | ConfigHost | `scieqlint.config` -> `scieqlint.cli`, `scieqlint.scan`, `scieqlint.parse`, `scieqlint.check`, `scieqlint.report` |
| `Owner: architecture boundary map - Resources must not import runtime analysis or presentation layers` | R1-002B planned | ResourceHost | `scieqlint.io.resources` -> `scieqlint.cli`, `scieqlint.scan`, `scieqlint.parse`, `scieqlint.check`, `scieqlint.report` |
| `Owner: architecture boundary map - Scanners must not import parser checker or reporter layers` | R1-002B planned | FrontendHost | `scieqlint.scan` -> `scieqlint.parse`, `scieqlint.check`, `scieqlint.report` |
| `Owner: architecture boundary map - Parsers must not import scanners checks reporters config loaders or SymPy` | R1-002B planned | MathHost | `scieqlint.parse` -> `scieqlint.scan`, `scieqlint.check`, `scieqlint.report`, `scieqlint.config.load`, `sympy` |
| `Owner: architecture boundary map - Checkers must not import scanner discovery reporter or CLI layers` | R1-002B planned | EngineHost | `scieqlint.check` -> `scieqlint.scan`, `scieqlint.io.discover`, `scieqlint.report`, `scieqlint.cli` |
...
| `scieqlint.engine` | current analysis engine interfaces and rule runners | EngineHost | `facts`, `query`, `diag.ir`, public source/query values | `cli`, `app`, `api`, `io`, `source`, `scan`, `frontend`, `report` | Not CompatibilityShell | No temporary exception; R1-002B contract `Owner: architecture boundary map - Engines consume query facts only` |
...
| `scieqlint.io.resources` | packaged resource loading | ResourceHost | `importlib.resources`, package data paths | `cli`, `scan`, `parse`, `check`, `report` | Not CompatibilityShell | No temporary exception; R1-002B planned contract `Owner: architecture boundary map - Resources must not import runtime analysis or presentation layers` |
...
| `scieqlint.report` | text, JSON, GitHub, SARIF renderers | SchemaHost projections | `diag.model`, public result/schema values, packaged schema metadata | `io` source loading, `source`, `scan`, `frontend`, `parse`, `check`, `engine` | Not CompatibilityShell | No temporary exception; R1-002B contract `Owner: architecture boundary map - Reporters render only diagnostics` |
| `scieqlint.scan` | current Markdown, LaTeX, notebook, and symbol scanners | FrontendHost | `io.source`, `diag.model`, `scan.base`, source-map/value helpers | `parse`, `check`, `report` | Transitional scanner facade for durable FrontendResult lowering | Owner: FrontendHost; retire direct legacy scanner DTO imports in R1-029/R1-031B/R1-032B |
| `scieqlint.schema` | current result projection model | SchemaHost | `diag`, `facts`, public projection values | scanners, parsers, checkers, CLI | Not CompatibilityShell | Owner: SchemaHost; retire ad hoc registry shape under R1-049 |
...
| Package or category | Current owner | Durable architecture owner | Allowed dependencies | Forbidden dependencies | CompatibilityShell status | Temporary exceptions |
|---|---|---|---|---|---|---|
| `tools/architecture` | Planned, no current checked-in package | Architecture conformance tooling | approved ownership map, import-linter config, committed fixtures | production analysis semantics, broad allowlists, unowned baselines | Not CompatibilityShell | R1-002B may create this only for executable gates |
