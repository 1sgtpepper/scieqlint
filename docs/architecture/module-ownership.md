...
The executable baseline currently covers the first six contracts. The remaining
...
| `Owner: architecture boundary map - FrontendHost owns notebook input mapping` | Current baseline | FrontendHost | `scieqlint.frontend` -> `scieqlint.scan` |
| `Owner: architecture boundary map - Frontends preserve candidates for semantic hosts` | Current baseline | FrontendHost | `scieqlint.frontend` -> `scieqlint.parse`, `scieqlint.policy`, `scieqlint.engine`, `scieqlint.check`, `scieqlint.report` |
| `Owner: architecture boundary map - MathHost and policy do not import frontends` | Current baseline | MathHost and PolicyHost | `scieqlint.parse`, `scieqlint.policy` -> `scieqlint.frontend`, `scieqlint.engine`, `scieqlint.check`, `scieqlint.report` |
...
| `scieqlint.app`, `scieqlint.app_profile*` | current orchestration service and profile compatibility adapters | AnalysisSession | `io`, `scan`, `parse`, `check`, `report`, `config`, `diag`, `facts`, `frontend`, `graph`, `policy`, `query`, `schema` | CLI command parsing and reporter-specific side effects outside orchestration | CompatibilityShell service facade | Owner: AnalysisSession; retire in R1-054A/R1-054B |
...
| `scieqlint.frontend` | MyST token lowering, notebook JSON input mapping, and structure extraction helpers | FrontendHost | `facts`, `ir`, `diag`, `source`, injected `io.workspace` path identity, narrow markdown-it token APIs, frontend-owned notebook JSON helpers | `parse`, `policy`, `engine`, `check`, `report`, `cli`, `scan`; direct engine orchestration | Not CompatibilityShell | `WorkspaceHost` is injected by orchestration; frontends do not discover or read files. Notebook DTO/span mapping is owned by `frontend.notebook_input`; the scanner may consume that owner through its transitional adapter. |
...
| `scieqlint.parse` | math grammar, AST, parser, normalization, printing | MathHost | `parse.ast`, `diag.model`, source-span types, Lark adapter | `scan`, `frontend`, `engine`, `check`, `report`, `config.load`, SymPy | Not CompatibilityShell | No temporary exception; future contract `Owner: architecture boundary map - Parsers must not import scanners checks reporters config loaders or SymPy` |
| `scieqlint.policy` | output-profile support, code-cell language-catalog policy, and diagnostic severity selection | PolicyHost preview | `facts`, `diag.catalog`, `diag.model` | `frontend`, `scan`, `parse`, `engine`, `check`, `report`, `cli`, `app`, `api` | Not CompatibilityShell | Preview scope only; suppression and baseline migration remain owned by R1-021A/R1-021B/R1-022/R1-023. |
...
| `scieqlint.scan` | current Markdown, LaTeX, notebook, and symbol scanners | FrontendHost | `io.source`, injected `io.workspace` path identity, `diag.model`, `scan.base`, source-map/value helpers, `frontend.notebook_input` transitional adapter | `parse`, `check`, `report` | Transitional scanner facade for durable FrontendResult lowering | `WorkspaceHost` is injected by orchestration; scanners do not discover or read files. Notebook scanner parsing delegates DTO/span mapping to the FrontendHost owner; `scan.notebook_input` and `scan.notebook_json` retain only compatibility re-exports. |
