...
normative import rules in [`../../SPEC.md`](../../SPEC.md#6-architecture), and
the R1-002B import-linter baseline issue. It does not rename packages, change
...
R1-002B must make these exact contracts executable. Existing contracts keep
their current executable names, including the `Owner: architecture boundary map`
owner prefix.
...
| `scieqlint.engine` | current analysis engine interfaces and rule runners | EngineHost | `facts`, `query`, `diag.ir`, public source/query values | `cli`, `app`, `api`, `io`, `source`, `scan`, `frontend`, `report` | Not CompatibilityShell | No temporary exception; current baseline contract `Owner: architecture boundary map - Engines consume query facts only` |
...
| `scieqlint.facts` | immutable fact records and snapshots | FactSnapshot | dataclasses, typing, `io.source` document value when needed | scanner/parser/checker execution, reporters, CLI | Not CompatibilityShell | None |
...
| `FactSnapshot` builder/indexing | Current facts and query values exist; builder/indexing completion planned | FactSnapshot and QueryHost | R1-037A/R1-037B/R1-040/R1-041A/R1-041B |
