"""Validated notebook input and exact raw-source location mapping."""
...
def parse_notebook_input(document: SourceDocument) -> NotebookInput:
    ...
    try:
        cell_source_ranges = _cell_source_ranges(document, cells, root_range)
...
def _cell_source_ranges(
    document: SourceDocument,
    cells: tuple[object, ...],
    root_range: tuple[int, int],
) -> tuple[tuple[tuple[tuple[int, int], ...], ...] | None, ...]:
    """Locate decoded cell-source characters in the original JSON."""
    ...
    return tuple(cell_source_ranges)
