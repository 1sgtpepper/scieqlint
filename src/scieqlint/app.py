...
def check_documents(
    ...
    *,
    ...
    if profile_documents:
        query = QueryHost(_profile_snapshot(profile_documents, config))
        if config.checks.references.enabled:
...
def _generated_profile_snapshot(
    ...
    snapshot = MathHost().classify(MySTFrontend().lower(markdown_documents))
    if config.profile.name == "notebook-crossrefs" and notebook_documents:
