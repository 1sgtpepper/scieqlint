...
class PortabilityQueryView:
    ...
    def notebook_rendering_conflicts(self) -> tuple[NotebookRenderingConflict, ...]:
        ...
        for cell in self.snapshot.code_cells:
            ...
            crossref_options = _crossref_options(cell, cell_outputs)
...
def _crossref_options(
    ...
    for output in outputs:
        ...
        options.update(key for key in output_options if key != "label")
        if "label" in output_options:
            has_crossref_label = True
    if has_crossref_label:
        return ("label", *sorted(options))
...
def _output_crossref_options(output: NotebookOutputFact) -> tuple[str, ...]:
    ...
    if label is not None and label.lower().startswith(_CROSSREF_PREFIXES):
        return ("label", *sorted(options))
    return tuple(sorted(options))
